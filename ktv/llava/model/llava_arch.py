...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images,keyframe_order=None, num_frames=None, prune_mode=None,rate=None,tokens_num=None):
        ...
        for i in range(len(image_features)):
            ...
            if prune_mode=='cls_new_token_sim' or prune_mode == 'uniform_token':
                ...
                cls_normalized_cosine_sim = (cls_sim-cls_sim.min()) / (cls_sim.max()-cls_sim.min())
                # 576. 
                features_normalized = image_feature / image_feature.norm(dim=1, keepdim=True)
                # token features
                ...
                cosine_sim = cls_normalized_cosine_sim*rate + (1-normalized_token_sim)*(1-rate)
    ...
    def prepare_ktv(self, image_features, temporal_aggregation):
        ...
        ktv_match = re.match(r'^ktv-slow_(\d+)frms_(\w+)-fast_(\d+)x(\d+)$', temporal_aggregation)
        if not ktv_match:
            raise ValueError(f'Failed to parse the temporal aggregation for ktv: {temporal_aggregation}')
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        images, image_sizes=None, temporal_aggregation=None,keyframe_order=None,num_frames=None,prune_mode=None,global_rate=None,
        tokens_num=None
    ):
