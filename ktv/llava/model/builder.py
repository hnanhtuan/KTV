...
def load_pretrained_model(
    ...
    **kwargs,
    ...
    if str(device) != "cuda" and device_map == "auto":
    ...
    if "llava" in model_name.lower():
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            mm_projector_weights = {
                k: v.to(dtype) for k, v in mm_projector_weights.items()
            }
            ...
            if "mpt" in model_name.lower():
                ...
                if rope_scaling_factor >= 2:
                    setattr(
                        cfg_pretrained,
                        "rope_scaling",
                        {"factor": float(rope_scaling_factor), "type": "dynamic"},
                    )
                    setattr(
                        cfg_pretrained,
                        "max_sequence_length",
                        4096 * rope_scaling_factor,
                    )
                    setattr(
                        cfg_pretrained,
                ...
                model = LlavaLlamaForCausalLM.from_pretrained(
                    model_path, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs
                )
    else:
        # Load language model
    ...
    if "llava" in model_name.lower():
        ...
        if kwargs["device_map"] != "auto":
            vision_tower.to(device=str(device), dtype=dtype)
