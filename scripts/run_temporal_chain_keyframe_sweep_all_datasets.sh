...
print_help() {
  cat <<'USAGE'
Usage:
  scripts/run_temporal_chain_keyframe_sweep_all_datasets.sh [options]

Runs temporal-chain experiments for every available QA dataset while sweeping
the keyframe count over 2, 3, 4, 5, and 6 from a single invocation.
This script uses temporal-chain Stage 1 keyframe extraction first, then runs
QA inference over those extracted frames without query-aware ranking or pruning.

Defaults:
  datasets: nextqa videomme intentqa egoschema
  variant: temporal_chain_keyframe_sweep
  counts: 2 3 4 5 6
  lambda-event (l): 0.7
  alpha-gap (a): 0.8
...
Strict options:
  ...
  --skip-existing-keyframes <0|1>  Skip a sweep count when its combined keyframe JSON already exists. Default: 0
  --output-root <path>             Root for dataset outputs. Default: outputs
  --variant-name <name>            Per-dataset output folder. Default: temporal_chain_keyframe_sweep
  --counts "<ints>"                Space-separated keyframe counts. Default: "2 3 4 5 6"
  ...
  --keyframe-impl <single|multiprocess>
                                   Temporal-chain script variant. Default: multiprocess
...
OUTPUT_ROOT="outputs"
VARIANT_NAME="temporal_chain_keyframe_sweep"
LAMBDA_EVENT="0.7"
ALPHA_GAP="0.8"
BETA_REDUNDANCY="0.6"
SCORE_NORMALIZER="minmax"
MAX_FRAMES_TO_EXTRACT="5400"
KEYFRAME_IMPL="multiprocess"
...
run_temporal_chain_count_for_dataset() {
  local dataset="$1"
  local num_keyframes="$2"
  local inference_experiment
  local dataset_root
  local count_dir
  local keyframe_dir
  ...
  count_dir="${dataset_root}/keyframes_${num_keyframes}"
  keyframe_dir="${count_dir}/per_question_keyframes"
  keyframe_json="${count_dir}/keyframe_order.json"
  pred_path="${count_dir}/${OUTPUT_NAME}.json"
  acc_path="${count_dir}/${OUTPUT_NAME}_accuracy.txt"
  hydra_log_dir="${count_dir}/hydra_resolved_configs"

  mkdir -p "${count_dir}" "${keyframe_dir}" "${hydra_log_dir}"

  echo "=================================================="
  echo "Dataset: ${dataset}"
  echo "Inference config: ${inference_experiment}"
  echo "Temporal-chain sweep dir: ${count_dir}"
  echo "Keyframe count: ${num_keyframes}"
  ...
  echo "Saved temporal-chain sweep keyframes to ${keyframe_json}"
...
for dataset in "${DATASETS[@]}"; do
  ...
  for num_keyframes in "${COUNTS[@]}"; do
    count_dir="${dataset_root}/keyframes_${num_keyframes}"
    run_with_experiment_logging "${count_dir}" "${dataset}_${VARIANT_NAME}_keyframes_${num_keyframes}" run_temporal_chain_count_for_dataset "${dataset}" "${num_keyframes}"
  done
done
