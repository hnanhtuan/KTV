...
print_help() {
  cat <<'USAGE'
Usage:
  scripts/run_temporal_chain_score_normalizer_sweep.sh [options]

Runs temporal-chain score normalizer sweep for Video-MME (or other datasets).

Defaults:
  datasets: videomme
  strategies: cluster_event lookahead
  normalizers: minmax percentile_minmax zscore_sigmoid robust_zscore_sigmoid rank softmax log_minmax
  num-keyframes: 12
  num-frames: 6
  tokens-num: 1872
...
Strict options:
  ...
  --normalizers "<names>"              Space-separated score normalizers.
...
run_setting() {
  ...
  export KTV_MLFLOW_TAGS_JSON="{\"workflow\":\"score_normalizer_sweep\",\"score_normalizer\":\"${normalizer}\",\"first_frame_strategy\":\"${strategy}\",\"dataset\":\"${dataset}\",\"output_dir\":\"${output_dir_abs}\"}"
...
for dataset in "${DATASETS[@]}"; do
  for strategy in "${STRATEGIES[@]}"; do
    for normalizer in "${NORMALIZERS[@]}"; do
      exp_id="normalizer_${normalizer}_strat_${strategy}"
      output_dir="${OUTPUT_ROOT}/${dataset}/score_normalizer_sweep/strategy_${strategy}/normalizer_${normalizer}"
      
      if [[ "${SKIP_EXISTING}" == "1" && -f "${output_dir}/predictions_accuracy.txt" && -s "${output_dir}/predictions_accuracy.txt" ]]; then
        echo "[SKIP] ${dataset} strategy=${strategy} normalizer=${normalizer} (already completed)"
        continue
      fi

      run_with_experiment_logging "${output_dir}" "${exp_id}" run_setting "${dataset}" "${strategy}" "${normalizer}" "${output_dir}"
    done
  done
done
