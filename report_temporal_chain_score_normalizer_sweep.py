"""Summarize temporal-chain score-normalizer sweep results."""
...
def discover_accuracy_files(root, output_name):
    ...
    return sorted(Path(root).glob(f"**/score_normalizer_sweep/**/{pattern}"))
...
def row_from_accuracy_path(path):
    ...
    for part in parts:
        ...
        elif part.startswith("normalizer_"):
            normalizer = part.removeprefix("normalizer_")
    
    # Try to find dataset name from path
    ...
    if not dataset:
        dataset = "unknown"
    return dataset, strategy, normalizer


def collect_rows(root, output_name):
    rows = []
    for path in discover_accuracy_files(root, output_name):
        dataset, strategy, normalizer = row_from_accuracy_path(path)
        if not strategy or not normalizer:
            continue
    ...
    rows.sort(key=lambda row: (row["dataset"], row["strategy"], -row["accuracy"], row["normalizer"]))
    return rows
...
def print_rows(rows):
    ...
    print("dataset\tstrategy\tnormalizer\taccuracy\tcorrect\ttotal\tunparsed")
    for row in rows:
        print(
            f"{row['dataset']}\t{row['strategy']}\t{row['normalizer']}\t"
...
def parse_args():
    ...
    parser.add_argument(
        "--root",
        default="outputs",
        help="Root folder to scan for score normalizer sweep accuracy files.",
    )
    parser.add_argument(
        "--output-name",
        default="predictions",
        help="Inference output stem used by the sweep script (e.g. predictions).",
    )
    parser.add_argument(
        "--output-csv",
        default="outputs/score_normalizer_sweep_summary.csv",
...
def main():
    ...
    with track_run(
        stage="temporal_chain_score_normalizer_report",
